...
{
    NODE *head;// points to the sll, pointer to the first node of the list
};
...
void insert_pos(LIST *ptr, int data, int pos) //insert at particular position
{
    ...
    while ((cur != NULL) && (i < pos)) // i<pos makes sure we got till last element and not more incase its insert last
    	//*****************************
    {
...
void delete_front(LIST *ptr) {
	...
	ptr->head = ptr->head->link;  // move head to next node (linked and stores address of next node) OR temp->link
...
void delete_duplicates(LIST *ptr) { // ordered list *************************** we [ut cur link in else cause if if executed one ele is removed so no need to inc cur then
	NODE *cur = ptr->head;
	while (cur && cur->link) {
